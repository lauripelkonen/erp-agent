...
class ProductMatcher:
    ...
    def _retry_llm_request(self, request_func, *args, **kwargs):
        ...
        - Model fallback order: GEMINI_MODEL -> gemini-2.0-flash -> gemini-2.5-flash-lite
        """
        max_retries = 5
        ...
        fallback_models = [
            Config.GEMINI_MODEL,  # Primary model (will be gemini-2.5-flash)
            'gemini-2.0-flash',   # First fallback
            'gemini-2.5-flash-lite'  # Second fallback
        ]
        ...
        for attempt in range(max_retries):
            ...
            if attempt >= primary_model_failures and current_model_index < len(fallback_models) - 1:
    ...
    def __init__(self, products_csv_path: Optional[str] = None, max_iterations: int = 5):
        ...
        # Initialize Lemonsoft API client for product searches (same as main.py)
        self.lemonsoft_client = LemonsoftAPIClient()
        self.logger.info("Lemonsoft API client created for product searches")
        ...
        try:
            ...
            self.group_based_matcher = GroupBasedMatcher()
        ...
        if os.path.exists(self.products_csv_path):
            try:
                ...
                self.products_df = pd.read_csv(self.products_csv_path, encoding="utf-8")
        ...
        if GroupBasedMatcher:
            try:
                self.group_based_matcher = GroupBasedMatcher(max_products_display=200)
    ...
    def _get_openai_embedding(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            raise
    async def match_terms_batch(self, term_dicts: List[Dict]) -> List[Dict]:
    ...
    # --------------------------- local search --------------------------
    async def _sql_search_by_searchcode(self, pattern: str):
        ...
        try:
            ...
            # In SQL LIKE, % is wildcard and _ is single character wildcard
            # We need to escape existing % and _ characters if they're meant literally
            sql_pattern = pattern
            
            # If pattern contains %, use it as-is for SQL LIKE
            if '%' not in pattern:
                # If no % wildcards provided, assume contains search and add % on both sides
                sql_pattern = f'%{pattern}%'

            self.logger.info(f"Searching product_searchcode with SQL pattern: '{sql_pattern}'")

            # Query to search product_searchcode field
            query = """
            SELECT 
                product_code as sku,
                product_description as name,
                product_description2 as extra_name,
                product_price as price,
                product_id as id,
                product_searchcode
            FROM products 
            WHERE product_searchcode LIKE ?
            """

            results = await self._execute_sql_query(query, [sql_pattern])
            
            if results:
                self.logger.info(f"Found {len(results)} products from SQL searchcode search")
                return results
            else:
                self.logger.info(f"No products found in SQL searchcode search for pattern '{sql_pattern}'")
                return []

        except Exception as e:
            self.logger.warning(f"SQL searchcode search failed for pattern '{pattern}': {type(e).__name__}: {e}")
            import traceback
            self.logger.debug(f"Full traceback: {traceback.format_exc()}")
            return []

    async def _local_wildcard_search_BACKUP(self, pattern: str):
        """Search for products from Lemonsoft ERP using pattern matching.
        
        Performs 4 searches in total:
        1. API search by name field
        2. API search by extra_name field  
        3. API search by sku field
        4. SQL search by product_searchcode field
        ...
        # Always try to use Lemonsoft API first, fallback to CSV on errors
        ...
        try:
            ...
            # Convert pattern from % wildcards to API format
            # Lemonsoft API uses % as wildcard (like SQL LIKE)
            # httpx will automatically URL-encode the % characters, so we use raw % here
            api_pattern = pattern
            if '%' in pattern:
                ...
                # If no % wildcards provided, assume contains search and add % on both sides
                api_pattern = f'%{pattern}%'

            self.logger.info(f"Searching Lemonsoft API for pattern: '{pattern}' -> API pattern: '{api_pattern}'")


            # Search in both name and extra_name fields
            search_results = []
            
            # Search in main product name
            try:
                self.logger.info(f"Searching in name field with pattern: '{api_pattern}'")
                

                response = await self.lemonsoft_client.get('/api/products', params={'filter.name': api_pattern})
                if response is not None and hasattr(response, 'status_code'):
                    if response.status_code == 200:
                        try:
                            ...
                            if data is not None and isinstance(data, dict):
                                ...
                                if results is not None and isinstance(results, list):
                                    # Extract essential fields including group_code for priority classification
                                    essential_results = []
                                    for product in results:
                                        if isinstance(product, dict):
                                            group_code = product.get('group_code')
                                    ...
                                    self.logger.info(f"Found {len(essential_results)} products searching by name with pattern '{api_pattern}'")
                                else:
                                    self.logger.info(f"API returned invalid results format when searching by name: {type(results)}")
                                    self.logger.info(f"Available keys in response: {list(data.keys())}")
                            else:
                        ...
                        self.logger.info(f"API returned status {response.status_code} when searching by name with pattern '{api_pattern}'")
                        # Try to get response text for debugging
                        try:
                            response_text = response.text if hasattr(response, 'text') else 'No response text'
                            self.logger.info(f"Response text: {response_text[:200]}...")
                        except:
                            pass
                else:
                    self.logger.info(f"API returned invalid response object when searching by name: {type(response)}")
            except Exception as e:
                self.logger.info(f"Exception during name search: {type(e).__name__}: {e}")
                import traceback
                self.logger.debug(f"Full traceback: {traceback.format_exc()}")

            # Search in extra_name field
            try:
                self.logger.info(f"Searching in extra_name field with pattern: '{api_pattern}'")
                response = await self.lemonsoft_client.get('/api/products', params={'filter.extra_name': api_pattern})
                if response is not None and hasattr(response, 'status_code'):
                    if response.status_code == 200:
                        try:
                            data = response.json()
                            if data is not None and isinstance(data, dict):
                                ...
                                if results is not None and isinstance(results, list):
                                    ...
                                    search_results.extend(essential_results)
                                    self.logger.info(f"Found {len(essential_results)} products searching by extra_name with pattern '{api_pattern}'")
                                else:
                        ...
                        self.logger.info(f"API returned status {response.status_code} when searching by extra_name with pattern '{api_pattern}'")
                        # Try to get response text for debugging
                        try:
                            response_text = response.text if hasattr(response, 'text') else 'No response text'
                            self.logger.info(f"Response text: {response_text[:200]}...")
                ...
                self.logger.info(f"Exception during extra_name search: {type(e).__name__}: {e}")
                import traceback
                self.logger.debug(f"Full traceback: {traceback.format_exc()}")


            # ADDITION: Search in product_searchcode field using SQL
            try:
                self.logger.info(f"Searching in product_searchcode field with SQL pattern: '{pattern}'")
                sql_results = await self._sql_search_by_searchcode(pattern)
                if sql_results:
                    ...
                    for sql_product in sql_results:
                        if isinstance(sql_product, dict):
                            product_code = sql_product.get('sku', '')
                            # Get group_code via API for priority classification
                            group_code = await self._get_group_code_for_product(product_code)
                            priority = self._classify_product_priority(group_code)
                    ...
                    self.logger.info(f"Found {len(sql_results)} additional products from SQL searchcode search with priority classification")
                else:
                    self.logger.info(f"No additional products found from SQL searchcode search")
            except Exception as e:
                self.logger.info(f"Exception during SQL searchcode search: {type(e).__name__}: {e}")
                import traceback
                self.logger.debug(f"Full traceback: {traceback.format_exc()}")

            # Remove duplicates by SKU (product code)
            unique_products = {}
            ...
            # Since Lemonsoft API already handles the wildcard filtering, we use all unique results
            filtered_products = list(unique_products.values())
            
            self.logger.info(f"After deduplication: {len(filtered_products)} unique products (from {len(search_results)} total results)")

            if not filtered_products:
                self.logger.info(f"No products found for pattern '{pattern}' (searched {len(search_results)} total results)")
                return None
            ...
            for product in filtered_products:
                if isinstance(product, dict):
                    ...
                    product_data = {
                        'Tuotekoodi': product.get('sku', ''),
                        'Tuotenimi': product.get('name', ''),
                        'extra_name': product.get('extra_name', ''),
                        'price': product.get('price', 0.0),
                        'id': product.get('id', ''),
                        'group_code': product.get('group_code'),
                        'priority': priority,
                        'Tuotenimi_lower': product.get('name', '').lower()
                    }
                    
                    if priority == 'priority':
                        priority_products.append(product_data)
                    else:
                        non_priority_products.append(product_data)

            # Create combined DataFrame with priority information
            all_products = priority_products + non_priority_products
            result_df = pd.DataFrame(all_products)
            
            # Log priority breakdown
            self.logger.info(f"Priority breakdown: {len(priority_products)} priority, {len(non_priority_products)} non-priority products")
            self.logger.info(f"Result DataFrame: {result_df}")
            self.logger.info(f"✅ Found {len(result_df)} products from Lemonsoft API for pattern '{pattern}'")
            return result_df

        except Exception as e:
            self.logger.warning(f"Lemonsoft API search failed for pattern '{pattern}': {type(e).__name__}: {e}")
            import traceback
            self.logger.debug(f"Full traceback: {traceback.format_exc()}")
    ...
    async def _search_by_product_codes(self, product_codes: list):
        """Search for products by their product codes directly from SQL database.
    ...
    async def _agentic_batch_match_with_context(self, term_dicts: List[Dict]):
        ...
        for retry_attempt in range(max_retries):
            try:
                ...
                # Step 2: Initialize group matcher
                try:
                    if self.group_based_matcher:
                        self.logger.debug("🔧 Initializing group matcher")
                        await self.group_based_matcher.lemonsoft_client.initialize()
                        self.logger.debug("✅ Group matcher initialized successfully")
                except Exception as e:
                ...
                try:
                    ...
                    contents = [
                        types.Content(
                            role="user",
                            parts=[types.Part(text="Begin batch matching.")]
                        )
                    ]
                    self.logger.debug("✅ Conversation initialized successfully")
                except Exception as e:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if response.candidates[0].content.parts[0].function_call:
                                ...
                                self.logger.info(f"💬 Batch agent called function: {function_name}")
                                ...
                                if function_result.get('response'):
                                    try:
                                        self.logger.debug("📤 Preparing function response for conversation")
                                        function_response_part = types.Part.from_function_response(
                                            name=function_call.name,
                                            response=function_result['response'],
                                        )
                                        
                                        # Append according to Gemini documentation
                                        contents.append(response.candidates[0].content)
                                        contents.append(types.Content(role="user", parts=[function_response_part]))
                                        self.logger.debug("✅ Function response added to conversation")
                                        
                                    except Exception as e:
                                        self.logger.error(f"❌ Error preparing function response: {e}")
                                        raise  # Re-raise to trigger retry
                            
                            else:
                                self.logger.warning(f"❌ No function call in Gemini response (iteration {iteration + 1})")
                                break
                    ...
                    for term_dict in term_dicts:
                        ...
                        if search_term in self.matched_products:
                            ...
                            matched_rows.append({
                                ...
                                "ai_reasoning": match_info.get("reasoning", "AI matched this product"),  # ADD AI REASONING
                                "ai_confidence": match_info.get("confidence", 75),  # ADD AI CONFIDENCE if available
                            })
                    ...
                    for term_dict in term_dicts:
                        ...
                        if search_term in self.matched_products:
                            ...
                            fallback_rows.append({
                                ...
                                "quantity": term_dict.get("quantity", "1"),
                                "explanation": term_dict.get("explanation", ""),
                                "ai_reasoning": match_info.get("reasoning", "AI matched this product before failure"),
                                "ai_confidence": match_info.get("confidence", 75),
                            })
    ...
    def _manage_conversation_context(self, contents: List[types.Content], max_tokens: int = 7000) -> List[types.Content]:
        ...
        # Process in pairs (model response + function result) to maintain coherence
        i = len(contents) - 1
        # Collect blocks to preserve internal order of pairs when reversing
        temp_blocks = []  # each block is a list of messages in chronological order within the pair/single
        ...
        while i > 0:
            ...
            # If this is a function response message, also get the preceding model message
            if i > 1 and hasattr(current_msg, 'parts') and current_msg.parts and any(getattr(p, 'function_response', None) for p in current_msg.parts):
                prev_msg = contents[i-1]
                prev_tokens = self._estimate_message_tokens(prev_msg)
                
                # Check if adding both messages would exceed limit
                if pruned_tokens + current_tokens + prev_tokens > max_tokens * 0.9:  # Leave 10% buffer
                    break
                
                # Add both messages as a block to preserve internal order (prev -> current)
                temp_blocks.append([prev_msg, current_msg])
                pruned_tokens += current_tokens + prev_tokens
                i -= 2  # Skip both messages
            else:
                # Single message (shouldn't happen often in function calling pattern)
                    ...
                    "properties": {
                        "matches": {
                            ...
                            "items": {
                                ...
                                "required": ["unclear_term", "product_code", "reasoning"]
                            }
                        }
                    },
                    "required": ["matches"]
    ...
    async def _execute_batch_function(self, function_name: str, function_args: Dict) -> Dict:
        ...
        if function_name == "search_by_product_codes":
            ...
            # Use SQL-based sorting for performance
            sorted_products = await self.group_based_matcher._sql_fetch_group_sorted(
                group_code, sort_by, limit=200
            )
            ...
            for match in matches:
                ...
                if unclear_term in self.all_products_context:
                    ...
                    if validation:
                        # Run quality check to get confidence score
                        match_quality_check = await self._validate_match_quality(unclear_term, product_code, None)
                        confidence = match_quality_check.get('confidence', 95)
    ...
    def _format_df_results(self, df) -> str:
        """Format DataFrame results for display, including stock and sales data."""
    ...
    async def _validate_product_code(self, product_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Fallback to direct SQL existence check
            try:
                query = (
...
Jos confidence < 70%, aseta is_good_match: false."""
            ...
            if not response or not response.text:
                return {'is_valid': True, 'confidence': 75, 'reason': 'Could not validate, accepting by default'}
            ...
            try:
                ...
                if 'false' in text.lower() or 'huono' in text.lower() or 'poor' in text.lower():
                    return {'is_valid': False, 'confidence': 50, 'reason': 'AI indicated poor match'}
                else:
                    return {'is_valid': True, 'confidence': 75, 'reason': 'AI quality check (fallback parsing)'}

        except Exception as e:
            self.logger.error(f"Error in match quality validation: {e}")
            # Default to accepting matches to avoid breaking the system
            return {'is_valid': True, 'confidence': 75, 'reason': f'Validation error: {str(e)}'}
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "confidence": min(100, max(0, int(result.get("confidence", 75)))),
                    "original_term": search_term
                }
