...
class ImageAnalyzer:
    ...
    def analyze_inline_images(self, filtered_emails: List[Dict]) -> List[Dict]:
        ...
        try:
            for email in filtered_emails:
                ...
                if inline_images:
                    self.logger.info(f"Found {len(inline_images)} inline images in email: {email['subject'][:50]}...")
                ...
                if attachments:
                    ...
                    if image_attachments:
                        self.logger.info(f"Found {len(image_attachments)} image attachments in email: {email['subject'][:50]}...")
    ...
    def _extract_image_to_temp_file(self, image_info: Dict) -> Optional[str]:
        ...
        Handles both standard attachments and multipart images
        ...
        try:
            ...
            # Some sources (e.g., Gmail API) provide raw bytes under 'data' instead of an attachment object
            ...
            if not attachment_obj and not gmail_bytes:
                self.logger.error("No attachment object for standard image")
                return None
