...
class GmailServiceAccountSender:
    ...
    def __init__(self):
        ...
        # Local development fallback: if Docker path doesn't exist, try local project root
        try:
            service_account_path = Path(self.service_account_file)
            if not service_account_path.exists():
                # Try local project root (3 levels up from this file: src/notifications/ -> project root)
                project_root = Path(__file__).resolve().parents[2]
                local_fallback = project_root / "config" / "gmail-service-account.json"
                if local_fallback.exists():
                    self.logger.info(
                        f"Service account file not found at {service_account_path}, "
                        f"using local fallback: {local_fallback}"
                    )
                    self.service_account_file = str(local_fallback)
        except Exception:
            pass  # If path resolution fails, continue with original path
    ...
    async def send_offer_confirmation(self, recipient_email: str, offer_details: Dict, 
        ...
        try:
            ...
            # Build product list with quality and specification
            product_list = []
            for i, product in enumerate(product_matches, 1):
                product_line = f"{i}. {product.get('product_name', 'N/A')}"
                if product.get('quality') or product.get('specification'):
                    details = []
                    if product.get('quality'):
                        details.append(f"Laatu: {product.get('quality')}")
                    if product.get('specification'):
                        details.append(f"Määrittely: {product.get('specification')}")
                    product_line += f" ({', '.join(details)})"
                product_list.append(product_line)

            product_list_text = '\n'.join(product_list) if product_list else 'Ei tuotteita'
...
- Allekirjoita PELKÄSTÄÄN: "//ERP Agent". ÄLÄ KÄYTÄ OLLENKAAN "ystävällisin terveisin" tai "best regards" tai jotain muuta.
- Sano allekirjoutuksen jälkeen, että virhetilanteessa ota yhteyttä lauri@erp-agent.com
