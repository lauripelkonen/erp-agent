...
import os
def get_our_companies() -> list:
    """
    Returns a list of 'our' company names (in various forms) depending on the context.

    The returned names are used for filtering out or recognizing internal companies.

    Returns:
        List[str]: List of internal company names (lowercase, normalized variants)
    """
    customer_name = os.getenv('CUSTOMER_NAME', '').upper()
    if customer_name == 'WABEK':
        return [
            'metec', 'lvi-wabek', 'lvi wabek', 'wcom group', 'wcom-group',
            'lviwabek', 'wcom', 'climapri', 'clima-pri'
        ]
    elif customer_name == 'STEN':
        return [
            'sten', 'kimet'
        ]
    return []