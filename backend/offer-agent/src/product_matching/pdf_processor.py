...
class PDFProcessor:
    ...
    def convert_pdf_to_images(self, attachment: Dict, max_pages: int = 2) -> List[Dict]:
        """
        Convert PDF pages to images for vision-based text extraction.
        
        This is used as a fallback when both Mistral OCR and standard PDF 
        extraction fail (e.g., for image-based PDFs or tricky formats).
        
        Args:
            attachment: PDF attachment dictionary with 'data' or 'attachment_object'
            max_pages: Maximum number of pages to convert (default: 2)
            
        Returns:
            List of image dictionaries with 'filename', 'data', 'mime_type', 'source'
        """
        filename = attachment.get('filename', 'unknown.pdf')
        self.logger.info(f"🖼️ Converting PDF to images: {filename} (max {max_pages} pages)")
        
        images = []
        temp_path = None
        
        try:
            # Get binary content from attachment
            content_data = self._get_attachment_binary_content(attachment)
            if not content_data:
            ...
            try:
                ...
                self.logger.info(f"📄 Converting {filename} pages to PNG images...")
                ...
                for i, pil_image in enumerate(pil_images, start=1):
                    ...
                    image_info = {
                        'filename': f"{Path(filename).stem}_page{i}.png",
                        'data': img_bytes,
                        'mime_type': 'image/png',
                    ...
                    self.logger.info(f"   ✅ Page {i}: {len(img_bytes)} bytes")
                
                self.logger.info(f"✅ Converted {len(images)} pages from {filename} to images")
                
            except ImportError:
                self.logger.warning("⚠️ pdf2image not available - trying PyMuPDF fallback")
                
                # Fallback to PyMuPDF (fitz) if pdf2image is not available
                try:
                    import fitz  # PyMuPDF
                    ...
                    for page_num in range(pages_to_process):
                        ...
                        image_info = {
                            'filename': f"{Path(filename).stem}_page{page_num + 1}.png",
                            'data': img_bytes,
                            'mime_type': 'image/png',
                            'source': 'pdf_conversion',
                            'original_pdf': filename,
                        ...
                        images.append(image_info)
                        self.logger.info(f"   ✅ Page {page_num + 1}: {len(img_bytes)} bytes")
                    
                    doc.close()
                    self.logger.info(f"✅ Converted {len(images)} pages from {filename} using PyMuPDF")
                    
                except ImportError:
                    self.logger.error("❌ Neither pdf2image nor PyMuPDF available for PDF-to-image conversion")
                    return []
                except Exception as e:
                    self.logger.error(f"❌ PyMuPDF conversion failed for {filename}: {e}")
                    return []
                    
        except Exception as e:
            self.logger.error(f"❌ Failed to convert PDF to images: {filename}: {e}")
            import traceback
